...
package virtualcircuit.components{
	...
	public class Voltmeter extends MovieClip{
		...
		var blackTarget:Object; //DropTarget object of the black pin
		var redTarget:Object; //DropTarget object of the red pin
		var voltageReading:Number; //Voltage between the redTarget and blackTarget
		var redPin:RedPin;	//Object of redPin
		var blackPin:BlackPin; //Object of blackPin
		var	redWire:MovieClip;	//Object of redWire
		var	blackWire:MovieClip; //Object of blackWire
		public var volt:VoltBody; //Object of voltBody
		var prevX:Number;	//previous x position of voltmeter
		var prevY:Number;	//previous y position of voltmeter
		var posX:Number;	//current x position of voltmeter
		var posY:Number;	//current y position of voltmeter
		var isDrag:Boolean;	//Boolean indicating the drag status of Voltmeter
		var isRedDrag:Boolean; 	//Boolean indicating the drag status of redPin
		var isBlackDrag:Boolean;	//Boolean indicating the drag status of blackPin
		var isInContact:Boolean; //Boolean indicating if either redPin or blackPin is inContact with a junction
		var gOps:GraphOps; //Object of GraphOps to perform graph operations
		var gf:GlowFilter; //Object of GlowFilter and is applied when the voltmeter is selected
		...
		//Listener that starts the drag of voltmeter 
		...
		//Set the voltmeter as selected and applies the filter
		...
		//Listener that ends the drag of voltmeter 
		...
		//Listener that ends the drag of voltmeter when the mouse up is on Stage and not on  the Voltmeter
		...
		//Converts the point pt from its coordinate space to that of to
		function localToLocal(fr:MovieClip, to:MovieClip,pt:Point):Point {
			return to.globalToLocal(fr.localToGlobal(pt));
		}
		// Sets the Visibility of the components of voltmeter
		...
		//sets up the voltmeter 
		public function setupVoltmeter(){
			var pt:Point=this.localToLocal(this.parent,this.volt.parent,new Point(640,500));
			this.volt.posX=pt.x;
			this.volt.posY=pt.y;
		...
		//sets up the pins of the voltmeter
		function setupPin(){
		...
		//Listerner that moves the pin when there is a mouse down on any of the pins
		...
		//This checks if any of the neighbours already has CheckForHit EnterFrame Event, if yes turns true else false
		function checkNeighbours(ng:Array):Boolean{
			var flag:Boolean=false;
			
			for(var i:int=0;i<ng.length;i++){
				if(ng[i].isCheckForHit){
					flag=true;
					break;
				}
		...
		//Listerner that stops the movement of the pin when there is a mouse up on any of the pins
		...
		//updates the voltage Reading between the redTarget and blackTarget
		public function updateVoltageReading(){
				...
				if((redTarget is Junction)&&(blackTarget is Junction)){
					if(redTarget.parent.subCircuitIndex==blackTarget.parent.subCircuitIndex){
						...
						if((redNode!=null && blackNode!=null) && redNode.subCircuitIndex!=-1){
							voltageReading=gOps.findRMSVoltage(redNode,blackNode);
		...
		//Returns the branch between the two nodes node1 and node2
		public function getBranch(node1:Node,node2:Node):XMLList{
...
//Listerner that draws the wire between the pins end and pin every time the pin is moved and is inside the Area
		...
		//set the position of voltmeter's body
		public function setPos():void{
			this.volt.x=this.volt.prevX;
			this.volt.y=this.volt.prevY;
		}
		
		//Checks if the voltmeter is inside the area and bounds it inside it
		function checkBoundArea():Boolean{
			
			var boundRight:Number=this.parent.boundArea.x+this.parent.boundArea.width/2;
			var boundLeft:Number=this.parent.boundArea.x-this.parent.boundArea.width/2-20;
			var boundUp:Number=this.parent.boundArea.y-this.parent.boundArea.height/2;
			var boundDown:Number=this.parent.boundArea.y+this.parent.boundArea.height/2;
