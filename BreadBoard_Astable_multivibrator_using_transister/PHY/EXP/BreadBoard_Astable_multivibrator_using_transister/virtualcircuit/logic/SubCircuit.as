package virtualcircuit.logic{
	...
	public class SubCircuit{
		...
		public static var cap1;
		public static var cap2;
		...
		public function nodalAnalysis():void{
			...
			if(flag){
			//setTransistor();
			if(componentArray.length==10 && resistorArray.length==4 && capasitorArray.length==2 && tranArray.length==2){
				circuiteOk=true;
				setTransistor();
		...
		function setBranchVoltage():void{
			...
			for(var i=0;i<branches.length;i++){
				...
				if(node1.voltage>=node2.voltage){
					...
					if(branches[i].type=="CROinputTerminal"){	
					     var node7=getNode(branches[i].startJunction.ids,1);
						var node8=getNode(branches[i].endJunction.ids,1);
			...
			for(var i=0;i<branches.length;i++){
				...
				/*if(branches[i].type=="bulb"){	
						...
						branches[i].innerComponent.glowBulb(icVoltOut);
					}*/
