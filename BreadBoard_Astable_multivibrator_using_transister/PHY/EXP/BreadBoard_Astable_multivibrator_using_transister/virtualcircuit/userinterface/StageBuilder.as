package virtualcircuit.userinterface{
	...
	public class StageBuilder {
		...
		static var resetButton:Button;
		static var deleteIcon:DeleteIcon;
		static var splitIcon:SplitIcon;
		var totGraphYPos=-105;
        var totGraphXPos=-100;
...
You can change the value by either inserting the desired value in the text box or by using the slider.  By selecting the All check box you change the values of all the objects of that particular component.  The ranges between which you can vary each of the property are: -
		...
		function loadComponents():void {
			  ...
			  //CRO Graph
				...
				componentBox.addChild(graphOutLineClip);
		...
		function outpt_Wave(graphScaleX:Number,graphScaleY:Number){
			...
			//pointsX.push(time1);
			//pointsY.push(0);
			//SubCircuit.icVoltOut=12;
			//trace("difference-"+time1*graphScaleX);
			//SubCircuit.icVoltOut=13;
			//SubCircuit.circuiteOk=true;
			if(SubCircuit.icVoltOut>0 && time1*graphScaleX>0.5 ){	
				X1=0;
				spike.graphics.clear();
				...
				var maxLimit=time1
				var t1=0
				for(var i=0;graphCount<=count;i++){
					
					if(i%2==0){
						
								
						         pointsX.push(minLimit);
								 var t1=Math.round(maxLimit*graphScaleX*10)/10;
								pointsX.push(t1);
								if(SubCircuit.circuiteOk==true){
									pointsY.push(0);
									pointsY.push(0);
								}else{
									 var intNum:Number = Math.floor(SubCircuit.icVoltOut); 
								     var decimal = parseFloat(String(SubCircuit.icVoltOut - intNum));
									 if(decimal>0){
									  
									  SubCircuit.icVoltOut=intNum+1;
								  	}
									pointsY.push(SubCircuit.icVoltOut*ghYScale*-1);
									pointsY.push(SubCircuit.icVoltOut*ghYScale*-1);
								}
								if(i==0){
									spike.graphics.moveTo(pointsX[0],pointsY[0]);
								}
						/*for(var j=minLimit;j<=maxLimit;j=j+.01){
								 var t1=Math.round(j*graphScaleX*10)/10;
								 
								 var intNum:Number = Math.floor(t1); 
								var decimal = parseFloat(String(t1 - intNum));
								 trace("intNum--" +intNum+ "decimal--"+decimal);
								 if(decimal>0){
						...
						minLimit=t1;
						maxLimit=maxLimit+time2;
					}else{
						        	 var t2=Math.round(maxLimit*graphScaleX*10)/10;
						         	 pointsX.push(minLimit);
								 	 pointsX.push(t2);
								 	 var intNum1:Number = Math.floor(SubCircuit.icVoltOut); 
								     var decimal1 = parseFloat(String(SubCircuit.icVoltOut - intNum1));
									 if(decimal1>0){
									    SubCircuit.icVoltOut=intNum1+1;
								  	}
								 
									pointsY.push(SubCircuit.icVoltOut*ghYScale*-1);
								 	pointsY.push(SubCircuit.icVoltOut*ghYScale*-1);
						/*for(var k=minLimit;k<=maxLimit;k=k+1){
								var t2=Math.round(k*graphScaleX*10)/10;
								 var intNum1:Number = Math.floor(t2); 
								var decimal1 = parseFloat(String(t2 - intNum1));
								 trace("intNum--" +intNum1+ "decimal--"+decimal1);
								if(decimal1>0){
									  t2=intNum1+1;
								  }
								//pointsX.push(t2);
								//pointsY.push(SubCircuit.icVoltOut*-1);
								
								
								
						}*/
						minLimit=t2;
								maxLimit=maxLimit+time1;
					}
					graphCount++;
					/*xPos=xPos*graphScaleX;
					pointsX.push(xPos);
					
					xPos++;
					//trace(timeString);
					if(time==i){
						graphchange=graphchange ? false:true
						if(graphchange){
							time=time2;
							timeString="secondTime";
						}else{
							time=time1
							timeString="firstTime";
						}
						//time = time ? time2:time1;
						//timeString=timeString ? "secondTime":"firstTime";
						
						graphCount++;
						i=0;
					}
					if(timeString=="firstTime"){
						pointsY.push(0);
					}else{
						pointsY.push((SubCircuit.icVoltOut*graphScaleY)*-1);
					}*/
					...
					/*trace(time1%i);
					if(time1%i!=0 && forTime1==true){
						 
						  pointsY.push(0);
						  
					}else{
						pointsY.push(SubCircuit.icVoltOut);
						if(forTime1!=true){
							if(time2%i==0){
								forTime1=true;
							}
				...
				spike.graphics.lineTo(pointsX[j],pointsY[j]);
				}
				
				
				
				
				//dataChart.dataProvider = [pointsX,pointsY];
			//pointsX[i]=X1;`
			//OutputWave=2*-Math.sin(20*3.14*30*pointsX[i])
			//pointsY[i]=OutputWave*5;
			//trace(pointsY[i]+"----------OutputWave");
			//X1=X1+.2;
			//spike.y=-100
		...
		function showBtnClicked(e:MouseEvent){
			...
			if(showBtn){
				...
				//if(showBtn)
					componentBox.addChild(dataChart);
					trace('addchild')
				lineArray1.dataProvider = pointsY;
				dataChart.categoryNames = pointsX;
				...
				componentBox.setChildIndex(graphOutLineClip,componentBox.numChildren-1);
				//componentBox.setChildIndex(dataChart,componentBox.numChildren-1);
				...
				graphOutLineClip.visible=false;
				//timer1.reset();
				//croArray.splice(0,croArray.length);
				//chartCategoryNames.splice(0,chartCategoryNames.length);
				//chartDataProvider.splice(0,chartDataProvider.length);
				//dataChart.clearTimeout(0);
				croVisible=false;
				//componentBox.removeChild(dataChart);
				...
				croShowButton.label="Show CRO";
				//timer1.removeEventListener(TimerEvent.TIMER,timer1_Fn);
			}
		}
		...
		public function loadTabComponents():TabbedPane{
			...
			deleteButton1=new Buttons("DELETE",640,520)		
			removeFromBoard=new Buttons("Remove From Board",640,495)	
			resistorScroll=new Scroll(690,392,"Resistance Resi:1000 Ohm",1000,50000,1);
			resistorScroll.name="resistorScroll";
			resistorScroll.slider.enabled=false;
			batteryScroll=new Scroll(690,427,"Battery Volt:12 V",12,15,1);
			batteryScroll.name="batteryScroll";
			batteryScroll.slider.enabled=false;
			/*capasitorScroll=new Scroll(690,462,"Capasitance::1 Micro F",1,10,0.01);
			capasitorScroll.name="capasitorScroll";
			capasitorScroll.slider.enabled=false;*/
			rotationScroll=new Scroll(690,362,"Component Rotation",0,360,1);
...
function addResistorToStage(e:MouseEvent):void {
		...
		function addCapacitorToStage(e:MouseEvent):void {
			...
			if(e.target.parent.parent is Branch){
				if (e.target.parent.parent.insideArea) {
					...
					capacitorObjNew.setPrev(645,228)
		...
		function addWireToStage(e:MouseEvent):void {
			...
			if(e.target.parent.parent is Branch){
				if (e.target.parent.parent.insideArea) {
					...
					e.target.parent.parent.startJunction.visible=true;
					e.target.parent.parent.endJunction.visible=true;
					e.target.parent.parent.scaleX=.9;
					e.target.parent.parent.scaleY=.9;
		...
		function changeScroll(e:MouseEvent){
			if(!(e.target.parent.parent is Branch)){
				...
				if(e.target.parent.parent.type=="resistor"){
					...
					resistorScroll.labelTxt.text="Resistance Resi:12 Ohm";
					}else{
				...
				if(e.target.parent.parent.type=="capacitorNew"){
					...
					if(isNaN(e.target.parent.capValue)){
						//capasitorScroll.labelTxt.text="Capacitance:1 Micro F";
					}else{
						//capasitorScroll.labelTxt.text="Capacitor Volt:"+e.target.parent.capValue+" Micro F";
					}
				}else{
				//capasitorScroll.slider.enabled=false;
				}
				
				if(e.target.parent.parent.type=="Transistor" || e.target.parent.parent.type=="capacitorNew" || e.target.parent.parent.type=="resistor"){
		...
		public static function removeObjFromBoard(){			
			if(selectedObj.type!= "CROinputTerminal"){
			if(selectedObj!=null){
		...
		function volt_right_FN(e:MouseEvent){
		  ...
		  count=componentBox.CRO.knobV_total.turner_volt.currentFrame;
		   ...
		   componentBox.CRO.knobV_total.turner_volt.gotoAndStop(count-1);
			    ...
			    componentBox.CRO.knobV_total.turner_volt.gotoAndStop(12);
		   }
			...
			/*if(count>0){
			count=count-4
			componentBox.CRO.knobV_total.turner_volt.gotoAndStop(count)
			}
			if(cnt2>=0){
			cnt2--
			}*/
		...
		function volt_left_FN(e:MouseEvent){
		   ...
		   componentBox.CRO.knobV_total.turner_volt.gotoAndStop(count+1);
			 ...
			 componentBox.CRO.knobV_total.turner_volt.gotoAndStop(1);  
		   }
		   ghYScale=componentBox.CRO.knobV_total.turner_volt.gScale;
		   pointsX.splice(0,pointsX.length);
			pointsY.splice(0,pointsY.length);
		...
		function time_left_FN(e:MouseEvent){
		   ...
		   componentBox.CRO.knobT_total.turner_time.gotoAndStop(count-1);
			    ...
			    componentBox.CRO.knobT_total.turner_time.gotoAndStop(19);
		   }*/
		...
		function time_right_FN(e:MouseEvent){
		   ...
		   componentBox.CRO.knobT_total.turner_time.gotoAndStop(count+1);
			    ...
			    componentBox.CRO.knobT_total.turner_time.gotoAndStop(1);
		   }*/
			  ghXScale=componentBox.CRO.knobT_total.turner_time.gXScale;
			/*if(count==0){
			count=48
