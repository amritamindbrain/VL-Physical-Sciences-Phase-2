package virtualcircuit.components {
	...
	public class Branch extends DynamicMovie{
		...
		public var baseJunction:Junction;
		...
		public function Branch(type:String) { 
			...
			if(this.type=="Transistor"){
				this.baseJunction=new Junction();
			}
			...
			if(this.type=="wire"){
				...
				this.endJunction.setPrev(this.innerComponent.width-this.endJunction.width/2+3,this.startJunction.y);
			...
			if(Circuit.isSchematic()){
				...
				if(this.type!="capacitorNew"  && this.type!="longWire" &&  this.type!="Transistor" &&  this.type!="CROinputTerminal"  && this.type!="TransistorBaseJunction" &&  this.type!="signalGenerator" &&  this.type!="inductor"){
			...
			this.baseJunction.addEventListener(MouseEvent.MOUSE_DOWN,startRotate);
			this.baseJunction.addEventListener(MouseEvent.MOUSE_UP,stopRotate);
				this.addChild(this.innerComponent);
			this.addChild(this.endJunction);
			this.addChild(this.startJunction);
			this.addChild(this.baseJunction);
			this.setChildIndex(this.startJunction,this.numChildren-1);
			this.setChildIndex(this.endJunction,this.numChildren-2);
			this.setChildIndex(this.baseJunction,this.numChildren-3);
			this.baseJunction.y=this.endJunction.y-26
			this.baseJunction.x=this.endJunction.x-26
			...
			/*if(this.type=="capacitorNew"){
				
			this.startJunction.addEventListener(MouseEvent.MOUSE_DOWN,this.startJunction.hitOnJunction);
			this.endJunction.addEventListener(MouseEvent.MOUSE_DOWN,this.endJunction.hitOnJunction);
		...
		private function getInnerComponent(type:String):CircuitComponent{
			...
			else if(type=="capacitorNew")
			{
				return new CapacitorNew();
			}
			...
			else if(type=="TransistorBaseJunction")
			{//return new Wire();
			  
				return new transistorBaseJunction();
				
			}
			else if(type=="CROinputTerminal")
			{
				
				return new CROinputTerminal();
				
			}
			else if(type=="signalGenerator")
			{
		...
		function startRotate(e:MouseEvent) {
			...
			if(this.type!="wire"){
				...
				/*trace("e.target=="+e.target.hitTestObject(midJunction));
				if(e.target is Junction){
					
				}else{*/
				
				if(e.target.parent.type!="signalGenerator" && e.target.parent.type!="CROinputTerminal" && e.target.parent.type!="BatteryShade"){
				e.target.startDrag();
				}
								//}
		...
		public function stopRotate(e:Event) {
			...
			//e.target.removeEventListener(Event.ENTER_FRAME,e.target.hitOnJunction )//COMMENTED
		...
		function stopAll(e:MouseEvent) {
				...
				if(this.startJunction.hasEventListener(Event.ENTER_FRAME)){
					this.startJunction.removeEventListener(Event.ENTER_FRAME,rotate);//COMMENTED
				...
				if(this.endJunction.hasEventListener(Event.ENTER_FRAME)){
					this.endJunction.removeEventListener(Event.ENTER_FRAME,rotate);//COMMENTED
		...
		public function scaleAll(xreal:Number,yreal:Number,xsc:Number,ysc:Number){
			...
			if(this.type!="capacitorNew" && this.type!="longWire"  &&  this.type!="Transistor" &&  this.type!="CROinputTerminal" && this.type!="TransistorBaseJunction" &&  this.type!="inductor" ){
