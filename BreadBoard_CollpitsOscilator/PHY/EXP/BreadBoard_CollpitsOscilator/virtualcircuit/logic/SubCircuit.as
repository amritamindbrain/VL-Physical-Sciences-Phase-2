package virtualcircuit.logic{
	...
	public class SubCircuit{
		...
		public function nodalAnalysis():void{
			...
			if(flag){
			//setTransistor();
			if(componentArray.length==13 && resistorArray.length==4 && capasitorArray.length==5 && tranArray.length==1 && inductorArray.length==1 && batteryArray.length==1 ){
				circuiteOk=true;
				setCircuite();
			...
			for(var p=0;p<branches.length;p++){
			    ...
			    if(branches[p].type=="capacitorNew"){
				   ...
				   if(capCount==2){
				      ...
				      cap2= branches[p].innerComponent.capValue;
				   }
